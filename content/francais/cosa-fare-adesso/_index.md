---
title: "Que faire maintenant — Français"
description: "Actions immédiates en cas d'urgence à Genzano di Roma : numéros à appeler, comportements à adopter, liens utiles."
layout: "single"
---

[← Retour à la vue d'ensemble en français](/francais/)

<div class="card border-danger mb-4">
<div class="card-body bg-danger bg-opacity-10 p-4">
<h2 class="h4 text-danger"><i class="bi bi-telephone-fill me-2" aria-hidden="true"></i>Appelez tout de suite</h2>
<div class="row">
<div class="col-md-6">
<p class="fs-3 fw-bold text-danger mb-1">112</p>
<p>Numéro Unique d'Urgence — pour toute urgence</p>
</div>
<div class="col-md-6">
<p class="fs-3 fw-bold text-danger mb-1">803 555</p>
<p>Salle Opérationnelle Protection Civile Latium</p>
</div>
</div>
</div>
</div>

## Que se passe-t-il ?

Identifiez la situation que vous vivez. Cliquez sur le lien correspondant pour voir les indications opérationnelles appropriées (pages en italien — utilisez le traducteur de votre navigateur si nécessaire) :

- 🌍 [**Tremblement de terre**](/rischi-prevenzione/rischio-sismico/) — secousse sismique ressentie
- 🌧️ [**Inondation**](/rischi-prevenzione/rischio-idrogeologico/) — eau haute, glissement de terrain
- 🔥 [**Incendie**](/rischi-prevenzione/rischio-incendio/) — feu de forêt ou urbain visible
- 💨 [**Vent fort**](/rischi-prevenzione/vento-forte/) — rafales violentes, chute de branches
- ⛈️ [**Orage violent**](/rischi-prevenzione/temporali-intensi/) — éclairs, grêle, pluies intenses
- ⚡ [**Coupure de courant**](/rischi-prevenzione/blackout/) — manque d'électricité

## Règles générales toujours valables

1. **Ne paniquez pas.** Pensez calmement et agissez en sécurité.
2. **Appelez le 112** pour signaler des situations de danger.
3. **N'entravez pas les secours.** Ne vous rendez pas sur les lieux par curiosité.
4. **Suivez uniquement les sources officielles.** Ne diffusez pas d'informations non vérifiées.
5. **Aidez les personnes en difficulté** si vous pouvez le faire en sécurité (personnes âgées, enfants, personnes en situation de handicap).
6. **Gardez votre téléphone chargé** et utilisez-le uniquement pour des communications nécessaires.

## Ressources

- [État de l'alerte météo actuelle](/allerte-meteo/)
- [Zones d'urgence — Plan Communal](/piano-emergenza/)
- [Cartographie opérationnelle](/cartografia/)
- [Contacts](/contatti/)
- [Kit d'urgence — que préparer](/rischi-prevenzione/kit-emergenza/)

## Kit consigliato per questo rischio

Per il rischio sismico tieni pronto un kit leggero e raggiungibile, utile sia se devi uscire rapidamente sia se devi restare in casa senza servizi.

- acqua in litri sufficienti per la famiglia;
- torcia con pile o ricarica funzionante;
- radio a pile o a manovella;
- power bank carico;
- fischietto per segnalare la presenza;
- scarpe chiuse, guanti da lavoro e piccola coperta termica;
- copie dei documenti, farmaci abituali e numeri utili.

Approfondisci nella pagina [Kit di emergenza: casa, evacuazione e auto](/rischi-prevenzione/kit-emergenza/).

