---
title: "Useful Numbers — English"
description: "All emergency numbers and useful contacts for citizens of Genzano di Roma."
layout: "single"
language: "en"
---

[← Back to English overview](/english/)

<div class="card border-danger mb-4">
<div class="card-body bg-danger bg-opacity-10 p-4">
<h2 class="h4 text-danger fw-bold"><i class="bi bi-telephone-fill me-2" aria-hidden="true"></i>In an emergency call 112</h2>
<p class="mb-0 mt-2"><span class="fs-1 fw-bold text-danger">112</span></p>
<p class="mb-0"><strong>Single European Emergency Number (NUE)</strong> — for any emergency: medical, public order, fires, technical rescue.</p>
</div>
</div>

## Emergency and rescue numbers

In the Lazio region, since 2017, **the only number to call in case of emergency is 112**. All calls reach the NUE Single Operations Centre, which dispatches the appropriate service (Fire Brigade, medical emergency, Carabinieri, Police, wildfires).

| Number | Service | When to call |
|---|---|---|
| **112** | Single Emergency Number (NUE) | Any emergency: medical, public order, fires, technical rescue |
| **803&nbsp;555** | Lazio Civil Protection Operations Room | Non-urgent reports and information on regional emergencies |

## Civil Protection Group contacts

These contacts are **for non-urgent information and requests**. In case of emergency, always call **112**.

- **Office:** Via Sicilia, 13-15 — 00045 Genzano di Roma (RM)
- **Phone:** [+39 06 9362600](tel:+39069362600)
- **Email:** [segreteria@protezionecivilegenzano.it](mailto:segreteria@protezionecivilegenzano.it)

## Useful apps

- **IT-alert** — National public alarm system: [it-alert.it](https://www.it-alert.it/en/)
- **Where ARE U** — App for 112 emergency calls with automatic location sending: [download the app](https://where.areu.lombardia.it/)

## In case of emergency

The Civil Protection Group **cannot be activated directly by citizens**. In case of emergency, contact **112** or **803&nbsp;555**: the competent authorities will alert and dispatch our teams.
