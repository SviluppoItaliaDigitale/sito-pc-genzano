...
| Parte 16 | Bozze social: gestione quota Gemini API | [`manuale/parte-16-bozze-social-gestione-quota-api.md`](manuale/parte-16-bozze-social-gestione-quota-api.md) |
...
| Parte 30 | Materiali pronti NotebookLM (`/risorse-pronte/`) — workflow completo dalla preparazione del pacchetto sul Desktop alla pubblicazione live sul sito. 8 passi operativi, agent dedicato `pc-materiali-publisher`, workflow CI `aggiorna-indice-ricerca.yml`, troubleshooting, licenza CC BY-NC-SA 4.0 | [`manuale/parte-30-materiali-pronti.md`](manuale/parte-30-materiali-pronti.md) |
| Parte 31 | Skill globali Claude Code (18 maggio 2026) — ~100 skill installate in `~/.claude/skills/` post cleanup conservativo. Policy invocazione obbligata col tool `Skill`. Routing rapido per task ≥3 step. Co-uso con agent custom `pc-*` in sequenza | [`manuale/parte-31-skill-globali-invocazione.md`](manuale/parte-31-skill-globali-invocazione.md) |
| Parte 32 | **"Approfondimenti video" multi-canale** (19 maggio 2026) — Sistema cross-match IDF-weighted fra 12 canali YouTube italofoni monitorati (5 tematici PC + 7 divulgativi con filtro lessicale ~250 keyword "PC in senso ampio"). Niente canali in lingua straniera. Risultato: sezione "Approfondimenti video" sfondo lavanda pastello AGID con 1-5 link YouTube pertinenti in fondo a ~91% delle pagine. Privacy-first (link, no embed). Aggiornamento mensile automatico | [`manuale/parte-32-approfondimenti-video-multicanale.md`](manuale/parte-32-approfondimenti-video-multicanale.md) |
| Parte 33 | **Autorevolezza (E-E-A-T), GEO/AI-SEO e microstampa** (21 maggio 2026) — pagina "Metodo editoriale", `llms.txt`, schema FAQPage opt-in (`faq_schema`), "Pagina rivista il" sulle guide pilastro, analytics GoatCounter (anonimo, senza cookie), IndexNow per indicizzazione rapida, file di verifica GSC/Bing nel repo, generatore microtext/microstampa (filigrane anti-falsificazione) integrato negli attestati | [`manuale/parte-33-autorevolezza-geo.md`](manuale/parte-33-autorevolezza-geo.md) |
| Parte 34 | **Catalogo dei giochi** (`/catalogo-giochi/`) — 28 maggio 2026, dopo audit ChatGPT. Catalogo informativo AGID dei 33 giochi educativi (10 infanzia + 13 primaria + 10 ragazzi), una scheda per gioco con fascia d'età, durata, obiettivo didattico, accessibilità WCAG 2.2 AA e CTA al gioco interattivo. Fonte unica di verità = manifest `CONTENUTI` di `static/giochi/assets/js/coach.js`. Parallelo all'Arena `/giochi/`, non sostitutivo. Shortcode `bottone-gioco` per CTA con `relURL` compatibile subpath GH Pages | [`manuale/parte-34-catalogo-giochi.md`](manuale/parte-34-catalogo-giochi.md) |
| Parte 35 | **Conoscere la Protezione Civile** (`/conoscere/`) — 29 maggio 2026. Livello dottrinale additivo: la PC spiegata come materia, complementare alle pagine operative. Voce di menu di primo livello (sincronizzata `hugo.toml` ↔ `site-chrome.js`). Pagine: Servizio Nazionale, le quattro fasi (previsione/prevenzione/soccorso/superamento + modello R = P×V×E), telecomunicazioni in emergenza (+ rete Zamberletti), vulcanismo dei Colli Albani (fonti INGV+DPC). Ogni fatto normativo verificato su Normattiva (D.Lgs. 1/2018 artt. 2,4,7,12,13,24,25); disclaimer divulgativo su ogni pagina; cross-link reciproco con le pagine operative e con 6 articoli `/comunicazioni/`. Scopribile da assistente virtuale e mappa del sito | [`manuale/parte-35-conoscere-protezione-civile.md`](manuale/parte-35-conoscere-protezione-civile.md) |
| Parte 36 | **Cruscotto del territorio** (`/cruscotto/`) — schede dati in tempo reale da fonti aperte (INGV terremoti/vulcani, radar DPC, satellite EUMETSAT, meteo Open-Meteo, **Meteo Europa ECMWF** auto-ospitata con data di validità, ItaliaMeteo, allerta, incendi EFFIS, aria Open-Meteo/ARPA/CAMS, emergenze EMS, mare). Commutatore a schede `dashboard-*`, privacy-first (no embed con cookie terzi dove possibile), monitoraggio fonti con `controllo-fonti-cruscotto.yml` | [`manuale/parte-36-cruscotto-territorio.md`](manuale/parte-36-cruscotto-territorio.md) |
| Parte 37 | **Laboratorio meteo** (`/laboratorio-meteo/`) — costruttore di grafici climatici interattivo (alfabetizzazione ai dati, anche per le scuole). Impianto ibrido: motore live ERA5/Open-Meteo (forecast fino a ieri + archivio per le serie lunghe) + esempi pre-cotti sui Castelli (`clima-*.json`, rigenerati mensilmente da `clima-castelli.yml`). Renderer SVG vanilla (no CDN), tabella dati equivalente + CSV, tooltip mouse/tocco/tastiera, privacy-first (Open-Meteo solo su clic) | [`manuale/parte-37-laboratorio-meteo.md`](manuale/parte-37-laboratorio-meteo.md) |
| Parte 38 | **Componenti Bootstrap Italia per i contenuti** — shortcode `callout` (nativo BI: info/avviso/pericolo/ok), `passi` (stepper numerato), `timeline` (linea del tempo), `galleria` (carosello accessibile per ≥4 foto, solo manuale WCAG 2.2.2); **indice di pagina con scrollspy** automatico (≥3 heading, opt-out `indice/toc:false`, sostituisce il vecchio TOC); utility `.pc-spinner` e `.table-sticky`. Quando usarli e quando no (sobrietà AGID) | [`manuale/parte-38-componenti-bootstrap-italia-contenuti.md`](manuale/parte-38-componenti-bootstrap-italia-contenuti.md) |
| Parte 39 | **Dossier interattivi** (`/dossier/`) — racconti visivi scrollytelling a tema scuro "spazio", full-bleed e accessibili (WCAG 2.2 AA, reduced-motion). "Motore" riusabile: sezione Hugo `dossier` + layout `single`/`list` + 9 shortcode (`dossier-hero`, `dossier-scena`, `dossier-dati`/`dossier-dato`, `dossier-confronto`, `dossier-hotspot`/`dossier-punto`, `dossier-chiusura`, `dossier-fonti`). Un nuovo dossier = un file in `content/dossier/`. Box homepage col dossier più recente, voce di menu sotto Risorse. Immagini solo con licenza chiara (NASA PD, Copernicus/ESA CC BY). Pilota: "La Terra vista dallo spazio" | [`manuale/parte-39-dossier-interattivi.md`](manuale/parte-39-dossier-interattivi.md) |
| Parte 40 | **Comunicazione di emergenza accessibile e interoperabile** (giugno 2026) — quattro novità per raggiungere chi ha bisogni comunicativi/linguistici e per rendere l'allerta machine-readable: (1) **feed CAP 1.2** `/allerta-cap.xml` (Common Alerting Protocol OASIS, output format Hugo da `data/allerta.json`, `identifier` byte-stabile anti-spam); (2) **tabelle CAA** `/tabelle-comunicazione/` (shortcode `caa-tabella`/`caa-voce`, board ARASAAC stampabili per afasia/disabilità cognitive/non parlanti); (3) **Piano familiare offline** (file HTML autoconsistente scaricato via Blob, apribile senza rete); (4) **Facile da leggere multilingua** (IT + en/eo/ro/ar). Più la **news-sitemap Google News** `/news-sitemap.xml` (articoli ultime 48h) | [`manuale/parte-40-comunicazione-emergenza-accessibile.md`](manuale/parte-40-comunicazione-emergenza-accessibile.md) |
| Parte 41 | **Le schede didattiche a più fogli, e come si rigenerano** (settembre 2026) — quattro schede stampabili non si scrivono a mano ma nascono da `scripts/schede-didattiche/`: l'alfabetiere in quaderno (26 fogli, una lettera per foglio, quattro scritture con font corsivo scolastico self-hosted), l'alfabetiere in tabella (1 foglio), i tre album da colorare (12 fogli ciascuno) e il memory da ritagliare (20 tessere, 10 coppie). Illustrazioni: pittogrammi **ARASAAC in bianco e nero** (`static/pittogrammi/arasaac-bn/` + `registro.json`, scaricati per identificativo), CC BY-NC-SA 4.0 con attribuzione **su ogni foglio**. Con la sequenza obbligata generatori → pacchetti → parità, e il conteggio delle pagine in stampa | [`manuale/parte-41-schede-a-piu-fogli-generate.md`](manuale/parte-41-schede-a-piu-fogli-generate.md) |
| Parte 42 | **Pubblicazione automatica su Instagram e Facebook** (settembre 2026) — i contenuti del sito escono da soli insieme all'articolo, appena la pagina è online, dal repository privato `social-pc-genzano`, che legge il materiale dal sito e non viceversa. Cosa non esce mai in automatico (`Allerta`, `Emergenza`, versioni facili, articoli non ancora online), commenti Instagram disattivati sui post automatici, menzioni degli enti e luogo, come fermare o spostare una pubblicazione dalla coda, cosa si può correggere dopo (su Facebook il testo sì, su Instagram no) | [`parte-42-pubblicazione-automatica-social.md`](manuale/parte-42-pubblicazione-automatica-social.md) |
...
Per altre AI o sessioni nuove: [`CONTESTO-PROGETTO.md`](CONTESTO-PROGETTO.md) (auto-generato da
`scripts/export-contesto-progetto.sh`) contiene tutto in un unico file pronto da incollare.
