...
| Parte 24 | Output Braille-ready per gli articoli — BRF (12 maggio 2026, Sera 1 MVP) — script Python `scripts/genera-braille.py` che genera file `.brf` ASCII puri per ogni articolo `content/comunicazioni/*.md` via liblouis + tabella `it-it-comp6.utb` (braille italiano 6 punti standard). Idempotente (mtime-based), CLI con `--force`/`--limit`/`--article`/`--verbose`, preprocessing Markdown + shortcode (foto/pittogramma/cosa-non-fare/chi-chiamare), paginazione 40×25 ICEB con form-feed e header `#N`. Output in `static/braille/comunicazioni/<slug>.brf` (gitignored). Sere 2-3 (deploy.yml integration + bottone download UI) pianificate | [`manuale/parte-24-output-braille-articoli.md`](manuale/parte-24-output-braille-articoli.md) |
