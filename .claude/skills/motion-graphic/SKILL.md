...
- Piper TTS, voce `it_IT-paola-medium` (scaricata da huggingface.co/rhasspy/piper-voices in `motion/.cache/piper`). `voce.py` legge `motion/voce/<nome>/testi.txt`: **una riga per scena**, nello stesso ordine di `CONFIG.scene`, `-` per le scene senza voce. Numeri in lettere, parole inglesi a orecchio (es. «pàuer banc»). Parametro: `--length-scale 1.08`.
- **Pause di lettura.** Da sola la voce quasi non si ferma sulla virgola (0–0,09 s misurati), poco sui due punti (0,28 s) e per niente sui puntini: gli elenchi escono tirati. `voce.py` legge la riga **intera** (così l'intonazione resta quella naturale: spezzarla non va, perché la voce chiude in discesa anche un pezzo che finisce con la virgola) e poi allunga la pausa dopo ogni segno: virgola 0,25 s · punto e virgola, due punti 0,40 · trattino 0,35 · punto, ? e ! 0,60 · puntini 0,70 (tabella `PAUSE`). Il punto esatto si trova allineando lo spettro del testo fino al segno, letto da solo, con quello della frase intera (DTW), e la pausa si inserisce nel silenzio che comincia lì, oppure, se la voce lega le parole, nel punto di minima energia con dissolvenze di 8 ms. **Si scrive la punteggiatura come la leggerebbe uno speaker**: una virgola dove serve respirare, i due punti prima di una spiegazione. Per una pausa **senza segno scritto** si usa `|` nel testo parlato (0,40 s; non compare nei sottotitoli, che stanno nella scena). Verificato il 24/09/2026 trascrivendo con un riconoscitore vocale ogni tratto fra due pause delle tre puntate: nessuna parola spostata da un tratto all'altro.
