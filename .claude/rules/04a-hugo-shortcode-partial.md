...
`static/monitor/index.html` (pagina statica a viste, fuori da Hugo) ha la vista **RADIO** (`data-v="rad"`, hash `#radio`, tasto `9`): ascolto delle bande radioamatoriali con **spettro e waterfall** via iframe **click-to-load** verso due OpenWebRX+ incorporabili (`RADIO_RX`: IZ0FKE Roma `https://sdr.noantri.org/`, predefinito; I6IQX Bucchianico `https://sdr-plus.i6iqx.it/` per 160/80/60/17/12 m e PMR446; `radioSetRx()` cambia origine e ricarica l'iframe), stesso pattern di `#windyWrap` (wrapper `#radioWrap`, `S.radio`, `radioSync()`), con pannello laterale di sintonia: gruppi HF/VHF/UHF/uso libero, bande con limiti «da … a …» (`RADIO_BANDE`, kHz, IARU R1 + PNRF), frequenze notevoli (`RADIO_PRESET`), sintonia manuale, selettore del ricevitore, link ai ricevitori solo-http della zona (`RADIO_LINK`, mai in iframe: mixed content), riquadro «e per trasmettere?». Regole:

- **Sintonia via hash, senza ricaricare**: `radioNav()` cambia solo l'hash dell'iframe (`#freq=Hz,mod=…[,secondary_mod=…]`); il client OpenWebRX+ ascolta `hashchange` e accetta l'hash **solo se la frequenza cade nel profilo SDR attivo** (nessun parametro URL per il profilo; un nuovo client riceve il primo dispositivo, il 2 m). Il pannello mostra sempre il profilo da selezionare nel ricevitore (`radioProf()`, stessa regola `|f − centro| ≤ banda/2` del client); al cambio profilo il ricevitore rilegge l'hash e si sintonizza da solo. Non "risolvere" il limite ricaricando l'iframe: interromperebbe l'audio.
- **Profili live**: `radioLive()` legge `status.json` (CORS aperto su IZ0FKE, assente su I6IQX → resta la copia incorporata) e aggiorna `profili` del ricevitore in uso; `radioProf(hz, rx)`/`radioAltRx(hz)` decidono copertura e suggerimento «Passa a …». CSP: entrambi gli host in `frame-src` **e** `connect-src` (rule 05). Fonti monitorate da `check-fonti-cruscotto.py`.
...
- La configurazione (ricevitore, frequenza, modo, gruppo, banda) è persistita in `pcgz-monitor-cfg`; l'accensione del ricevitore no (click-to-load esplicito). Documentazione utente: `manuale/parte-36` § 36.8.
