...
Richiesta dell'utente: *«voglio sapere tutti i satelliti, con filtro per quelli ambito protezione civile e tutti i filtri possibili»*. Vista `data-v="orb"` (slug `satelliti`) + layer di cartina `data-l="orb"` (🔴 **non** `sat`, già usato dalle immagini satellitari EUMETSAT).
...
Dal 22/09/2026 il timbro lo risolve la CI: **`scripts/genera-dpc-bollettino.py`** (step di `aggiorna-dati-sala.yml`, ogni 15 minuti, con `GITHUB_TOKEN` → mille richieste l'ora) scrive `static/open-data/dpc-bollettino.json` con `stamp` e `name`, e `findStamp()` legge quello con **una richiesta sola e senza quota**. Le due carte topojson restano su `raw.githubusercontent.com`, che non ha quel limite ed è già in CSP. L'API resta come **ripiego** per l'ambiente locale prima del primo deploy.
...
🔴 **I tasti 1-9 NON seguono l'ordine della barra** (`order` nel gestore `keydown`): aprono ALLERTA, SISMICO, METEO, CARTE, WINDY, TRAFFICO, ARIA·MARE, EMERGENZE, REGISTRO, mentre MIA SALA, NAVI, RADIO e SATELLITI non hanno scorciatoia. L'elenco si ferma a nove perché la condizione è `e.key>="1"&&e.key<="9"`: un decimo elemento non lo raggiungerebbe nessuno (ce n'era uno, `"rad"`, rimosso il 22/09/2026 — prometteva una scorciatoia mai esistita). Aggiungendo una vista **non** si è obbligati a darle un tasto; se glielo si dà, si sposta ciò che viene dopo, e chi usa la sala in sede ha la memoria delle dita: è una scelta da concordare, non da fare di passaggio.
