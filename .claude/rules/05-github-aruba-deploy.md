...
> 🔴 **VIETATO il bump del `state-name` (regola invertita il 03/07/2026 — su questo sito NON funziona).** Fino a giugno 2026 il rimedio documentato per il drift grave era cambiare il `state-name` nello step FTP di `deploy.yml` per forzare il re-upload integrale. **Non farlo mai più**: il sito conta ~25.000 file (di cui migliaia di micro-file dell'indice Pagefind da ~5 KB) e su FTP Aruba ogni file è una connessione dati completa (~0,3 s) — l'upload integrale supera i 90 minuti di timeout e **non completa mai**. Il 03/07/2026 un bump ha innescato un loop di upload integrali mai finiti → sync-state mai salvato → **sito congelato per l'intera giornata** (12 deploy falliti, articolo da calendario mai andato live). Il commento nello step FTP di `deploy.yml` (righe ~150-168) è la fonte di verità di questo divieto.
>
> **Rimedio corretto per il drift**: fix **MIRATO** sulle singole pagine stantie — toccare il file sorgente perché l'HTML cambi e venga ri-caricato (cache-bust nei `_index.md`, vedi sotto), oppure cancellare il file stantio lato server così l'azione lo ri-carica. Mai re-upload integrale, mai `dangerous-clean-slate: true` (distruggerebbe `/documenti/` gestita a mano). 🟢 **Rilevamento proattivo (01/07/2026):** ogni pagina Hugo porta un fingerprint di build (`<meta name="pc-build-sha">` da `data/buildinfo.json`, scritto in CI da `deploy.yml`); il workflow `verifica-deploy-aruba.yml` lo campiona sulle pagine live dopo ogni deploy e apre un'issue `urgente` appena rileva **drift di generazioni** (pagine servite da build diverse) o un sito congelato. Alla issue, applica il fix mirato qui sopra sulle pagine segnalate. Il check `audit-sito.yml § 43b` confronta il livello allerta home vs `/allerte-meteo/` e segnala proprio questo sintomo.
