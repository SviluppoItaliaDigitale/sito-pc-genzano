...
| Parte 24 | Output Braille-ready per gli articoli — BRF (12 maggio 2026, Sera 1 MVP) — script Python `scripts/genera-braille.py` che genera file `.brf` ASCII puri per ogni articolo `content/comunicazioni/*.md` via liblouis + tabella `it-it-comp6.utb`. Idempotente, CLI ricca, preprocessing Markdown + shortcode (foto/pittogramma/cosa-non-fare/chi-chiamare), paginazione 40×25 ICEB. Output in `static/braille/comunicazioni/<slug>.brf` (gitignored). Sere 2-3 (deploy.yml + bottone download UI) pianificate | [`parte-24-output-braille-articoli.md`](parte-24-output-braille-articoli.md) |
