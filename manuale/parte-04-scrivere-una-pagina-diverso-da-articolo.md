...
| Per le scuole ▾ | dropdown | Formazione e scuole, Percorsi didattici pronti, Schede didattiche stampabili, Per i docenti — Ed. Civica, Storie e Racconti, Giochi della Sicurezza |
...
**Radar pioggia DPC — integrazione nativa WMS** (shortcode `radar-dpc.html`): usa i servizi **OGC WMTS** ufficiali del Dipartimento (`radar-geowebcache.protezionecivile.it`, layer `radar:sri`/`radar:vmi`, gridset EPSG:900913) in una mappa **Leaflet self-hosted** + base OpenStreetMap (variante scura via filtro CSS `.osm-scura`, toggle chiara), niente iframe di terzi. _(Dal 28/08/2026 niente più tile CARTO: il servizio anonimo marchia le tile con "API KEY REQUIRED".)_ L'orario reale del dato viene letto dall'API `findLastProductByType`. ⚠️ Il WMTS pubblico **ignora il parametro TIME** (nessuna animazione storica via tile): si mostra l'ultimo fotogramma. È un mosaico **grezzo**, con possibili echi non meteorologici ai bordi della copertura (nota nello shortcode + link al radar validato).
...
### 4.9-ter — Cruscotto del territorio (`/cruscotto/`) — 16 schede native (aggiornato 27/05/2026)
...
| Aria Europa (CAMS) | `dashboard-cams` | Copernicus CAMS via WMS pubblico ECMWF `eccharts.ecmwf.int/wms/?token=public` (no auth, no API key) | Forecast orario europeo: PM2.5/PM10/O₃/NO₂/SO₂/polveri sahariane, orizzonti ora/+6h/+24h/+48h/+72h |
| Emergenze EU (EMS) | `dashboard-ems` | Copernicus EMS Rapid Mapping JSON API `rapidmapping.emergency.copernicus.eu` (CORS aperto, no auth) | Ultime 15 attivazioni mappature emergenza in Europa, filtrabili per categoria (incendi/alluvioni/tempeste/terremoti/altro) e per area (tutta EU / solo Italia) |
