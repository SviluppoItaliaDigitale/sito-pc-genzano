...
REV = "Rev. 2 · 15/09/2026"
PITTO = "/pittogrammi/arasaac-bn/"
USCITA = pathlib.Path(__file__).resolve().parent.parent / (
...
def _eti(x, y, testo, corpo=2.8, colore=NERO, spazio=None, peso="bold"):
    if spazio:
        _sta(testo, corpo, spazio)
    return (f'<text x="{x}" y="{y}" text-anchor="middle" font-size="{corpo}" '
            f'fill="{colore}" font-weight="{peso}">{testo}</text>')


def fustella(scena_file, scena_alt, etichetta):
    """Il gradino: si ritaglia il rettangolo, si piega a metà, due tagli, una spinta.

    Geometria identica su tutte e tredici le tavole: cambia solo la figura. I
    numeri sul disegno sono gli stessi tre della guida, così il bambino non deve
    tradurre niente. Nessuna misura in millimetri: il pezzo si ritaglia seguendo
    le righe stampate, quindi la stampa «adatta alla pagina» non rovina nulla.
    """
    sx, dx = 36, 72          # i due tagli
    piega, fondo = 30, 48    # piega a metà del pezzo, fine dei tagli
    return f'''<svg class="fust" viewBox="0 0 108 74" role="img"
     aria-label="Pezzo da ritagliare: un rettangolo con la piega a metà, due tagli che partono dalla piega e una linguetta da spingere in avanti, con sopra {scena_alt.lower()}"
     xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
  <rect x="6" y="6" width="96" height="54" stroke="{NERO}" stroke-width="1" fill="none" rx="1"/>
  {_eti(54, 4.2, "1 · RITAGLIA IL RETTANGOLO", 3, NERO, spazio=104)}
  <line x1="6" y1="{piega}" x2="102" y2="{piega}" stroke="{BLU}" stroke-width="1" stroke-dasharray="3 2"/>
  {_eti(54, 27.5, "2 · PIEGA QUI, A META&#x27;", 3, BLU, spazio=96)}
  <line x1="{sx}" y1="{piega}" x2="{sx}" y2="{fondo}" stroke="{NERO}" stroke-width="1.2"/>
  <line x1="{dx}" y1="{piega}" x2="{dx}" y2="{fondo}" stroke="{NERO}" stroke-width="1.2"/>
  <line x1="26" y1="39" x2="{sx - 1.5}" y2="39" stroke="{NERO}" stroke-width="0.4"/>
  <path d="M{sx},39 l-2.2,-1.1 l0,2.2 Z" fill="{NERO}"/>
  {_eti(18, 40, "3 · TAGLIA", 3, NERO, spazio=36)}
  <line x1="82" y1="39" x2="{dx + 1.5}" y2="39" stroke="{NERO}" stroke-width="0.4"/>
  <path d="M{dx},39 l2.2,-1.1 l0,2.2 Z" fill="{NERO}"/>
  {_eti(90, 40, "3 · TAGLIA", 3, NERO, spazio=36)}
  <image xlink:href="{PITTO}{scena_file}" x="41" y="{piega + 1}" width="26" height="16" preserveAspectRatio="xMidYMid meet"/>
  <line x1="{sx}" y1="{fondo}" x2="{dx}" y2="{fondo}" stroke="{ARANCIO}" stroke-width="0.9" stroke-dasharray="0.8 1.6" stroke-linecap="round"/>
  {_eti(54, 52.5, "4 · SPINGI IN AVANTI QUESTA LINGUETTA", 3, ARANCIO, spazio=96)}
  {_eti(54, 58.5, etichetta, 4, NERO, spazio=96)}
  {_eti(54, 68, "Coloralo prima di ritagliare. Niente colla.", 3, GRIGIO, spazio=104, peso="normal")}
</svg>'''


def mini_passi():
    """I tre passi del montaggio, disegnati. Gli stessi per ogni tavola."""
    # La piega è orizzontale come nelle tavole: se qui fosse verticale, il
    # bambino girerebbe il foglio e i tagli non partirebbero dalla piega.
    p1 = f'''<svg viewBox="0 0 60 40" role="img" aria-label="Il foglio si piega a metà lungo la riga blu orizzontale">
      <rect x="6" y="5" width="48" height="30" stroke="{GRIGIO}" stroke-width="0.6" fill="none" rx="1"/>
      <line x1="4" y1="20" x2="56" y2="20" stroke="{BLU}" stroke-width="1" stroke-dasharray="3 2"/>
      <path d="M46,9 q7,6 0,11" stroke="{NERO}" stroke-width="0.8" fill="none"/>
      <path d="M46,20 l1.2,-2.4 l-2.4,0 Z" fill="{NERO}"/></svg>'''
    p2 = f'''<svg viewBox="0 0 60 40" role="img" aria-label="Si tagliano due righe nere che partono dalla piega">
      <rect x="12" y="6" width="36" height="28" stroke="{GRIGIO}" stroke-width="0.6" fill="none" rx="1"/>
      <line x1="12" y1="6" x2="48" y2="6" stroke="{BLU}" stroke-width="1.2"/>
      <line x1="24" y1="6" x2="24" y2="20" stroke="{NERO}" stroke-width="1.2"/>
      <line x1="36" y1="6" x2="36" y2="20" stroke="{NERO}" stroke-width="1.2"/>
      <line x1="24" y1="20" x2="36" y2="20" stroke="{ARANCIO}" stroke-width="0.8" stroke-dasharray="0.8 1.6"/></svg>'''
    p3 = f'''<svg viewBox="0 0 60 40" role="img" aria-label="Aprendo il foglio la linguetta si spinge in avanti e diventa un gradino in rilievo">
      <path d="M8,32 L30,26 L52,32" stroke="{GRIGIO}" stroke-width="0.6" fill="none"/>
      <path d="M8,32 L8,12 L30,6 L30,26 Z" stroke="{GRIGIO}" stroke-width="0.6" fill="#f8fafc"/>
      <path d="M52,32 L52,12 L30,6 L30,26 Z" stroke="{GRIGIO}" stroke-width="0.6" fill="#fff"/>
      <path d="M22,24 L38,24 L38,16 L22,16 Z" stroke="{NERO}" stroke-width="0.9" fill="#fff"/>
      <path d="M30,24 L30,16" stroke="{ARANCIO}" stroke-width="0.6" stroke-dasharray="0.8 1.6"/></svg>'''
    passi = [
        ("Ritaglia e piega a metà", "Ritaglia il rettangolo, poi piegalo sulla riga blu. Passaci sopra l'unghia.", p1),
        ("Taglia le due righe nere", "Partono dalla piega. Ferma le forbici dove comincia la riga arancione.", p2),
        ("Apri e spingi in avanti", "Apri il pezzo e spingi la linguetta verso di te: il gradino si alza da solo.", p3),
    ]
    out = []
  ...
  <article class="scheda-page" id="{t["id"]}" data-scelta="{t["id"]}">
    <div class="lib">
      ...
      <div class="lib-fustella">
        ...
        rettangolo, piegalo a metà sulla riga blu, taglia le due righe nere partendo dalla piega,
        apri e spingi in avanti la linguetta. Niente colla e niente misure da rispettare.</p>
      </div>
      {tessere}
...
def pagina_guida():
  ...
  <article class="scheda-page" id="guida" data-scelta="guida">
    <div class="lib">
      {intestazione("Come si fa", "Tre passi, uguali per tutte le tavole")}
      <p class="lib-tag">SI IMPARA UNA VOLTA SOLA</p>
      <p class="lib-guida-intro">Tutte le tavole di questo libro si montano nello stesso modo.
      Non c'è colla, non ci sono alette da incollare e non serve il fermacampione: si ritaglia, si
      piega a metà, si taglia due volte, si spinge. Quando hai fatto la prima tavola, sai fare anche le altre.</p>
      {mini_passi()}
      <div class="lib-guida">
        <div class="lib-box">
        ...
        <div class="lib-box">
          ...
          <table class="lib-legenda">
            <tr><td><span class="tr-taglio"></span></td><td><strong>Riga nera</strong> — qui si taglia.</td></tr>
            <tr><td><span class="tr-monte"></span></td><td><strong>Riga blu</strong> — qui si piega il foglio a metà.</td></tr>
            <tr><td><span class="tr-valle"></span></td><td><strong>Riga arancione</strong> — qui si piega la linguetta e si spinge.</td></tr>
          </table>
          <p class="lib-trucco"><strong>Il trucco:</strong> prima di piegare, passa l'unghia o una
          penna scarica sulla riga, con il righello. La carta si schiaccia e la piega viene dritta
          invece di strapparsi.</p>
        </div>
      </div>
      <div class="lib-box lib-box-largo">
        <h3>Se vuoi fare di più (facoltativo)</h3>
        <p class="lib-extra">Fai un <strong>secondo gradino più piccolo</strong> dentro al primo:
        due tagli più corti, sempre partendo dalla piega, e un'altra spinta. Vengono due scene una
        dietro l'altra. È lo stesso gesto di prima, quindi non c'è niente di nuovo da imparare.</p>
      </div>
      ...
      100%. Una tavola per volta è il ritmo giusto: montarle tutte in un pomeriggio stanca e il
      libro viene peggio.</p>
      {piede("Guida")}
    </div>
  </article>'''
...
CSS = """
    ...
    .lib-passo { flex: 1; border: 1px solid #cbd5e1; border-radius: 3px; padding: 2.5mm;
    ...
    .lib-passo-dis svg { width: 42mm; height: 28mm; }
    .lib-passo-testo strong { display: block; font-size: 11px; color: var(--scheda-blu);
      margin-top: 1mm; }
    .lib-passo-testo span { font-size: 9.5px; line-height: 1.35; }
