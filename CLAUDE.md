...
9. **FOTO FORNITE DALL'UTENTE — BANNER INTOCCABILE**: le foto fornite dall'utente **NON vanno MAI nel banner/copertina** dell'articolo (campo `image:` del frontmatter). Il banner in alto deve restare **pulito** col solo titolo + page-hero blu istituzionale: è una scelta di design che non si tocca. Le foto vanno **tutte dentro il corpo articolo** con lo shortcode `{{< foto >}}` (mai markdown `![]()` diretto), con fascia blu istituzionale e alt text significativo. Per **foto multiple in articoli storici** seguire la convenzione: 1ª dopo 1° H2, 2ª dopo 2° H2, foto successive sull'H2 di ogni evento specifico citato. **≥4 foto → galleria/carosello dentro l'articolo** (CSS scoped, cliccabili). Il campo `image:` resta `""` (il workflow `auto-cover-mancanti.py` genera la cover tipografica automatica per og:image e archive list) oppure è popolato da foto Wikipedia/NASA/USGS via marker `# TODO-foto-*`. **Sui social è diverso**: lo script `genera-immagini-social.py` legge le foto inline `{{< foto >}}` dal body e le combina automaticamente in **carosello Instagram** — stessa fonte, due usi distinti, niente da configurare. Mai foto a casaccio.
