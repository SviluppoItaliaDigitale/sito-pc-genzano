...
- **Mai** il trailer `Co-Authored-By: Claude ...` né righe `Claude-Session: ...` nei messaggi di commit. 🟢 **Dal 21/09/2026 il divieto è applicato da un hook**, non più dalla memoria: `.githooks/commit-msg` toglie quelle righe prima che il commit esista (e rifiuta il messaggio se dopo la pulizia non resta nulla). Lo attiva `scripts/imposta-identita-git.sh` puntando `core.hooksPath` a `.githooks`. Nasce da un censimento che ha trovato **170 commit su `main` dal 19 giugno 2026** con la riga di sessione: ripulirli richiederebbe di riscrivere oltre 5.000 commit, quindi restano e la difesa vale per il futuro. In `.claude/settings.json` c'è `"includeCoAuthoredBy": false`; nelle sessioni (es. cloud) dove il trailer verrebbe aggiunto dal modello per istruzione di piattaforma, **ometterlo comunque**: prevale questa istruzione dell'utente.
...
| **Slide social** (`social_citazione` / `social_punti`) | Compilati DAL testo dell'articolo, **mai inventati**: `social_citazione` = una frase forte/chiave dell'articolo (tra virgolette); `social_punti` = 3-4 punti chiave (date, luoghi, azioni di autoprotezione, numeri utili). Alimentano le slide "citazione" e "In sintesi" del carosello social (`genera-immagini-social.py`). Ometto se l'articolo è troppo breve o privo di contenuto sintetizzabile (es. comunicato di una riga). **`social_video`** (dal 23/09/2026): se l'articolo è fatto di video, `social_video: "/video/<nome>.mp4"` fa uscire un **Reel** su Instagram e un **post video** su Facebook al posto delle immagini; più video si uniscono in un unico MP4 verticale 720×1280 (H.264 Main 3.1, traccia audio anche muta), perché un articolo è un post e il carosello ritaglierebbe il 9:16. |
| **Orari degli interventi** | 🔴 **Sempre arrotondati**, mai al minuto ("verso le 18", "poco prima delle 10", "in tarda serata", "fin oltre le 23") — il minuto esatto resta nel registro interno, non negli articoli (rule 10 § promemoria export interventi; richiamo utente 28/08/2026 dopo un articolo live con 9:44/18:04/23:10). |
| **Fonti giornalistiche** | 🔴 **Sempre parafrasate, mai citate nel corpo**: niente "(Castelli Notizie)"/"(Il Mamilio)" inline, niente virgolettati attribuiti a testate, niente frasi copiate. Nel corpo si riprendono **solo i fatti** che il nostro registro non ha, riscritti con parole nostre (attribuzione generica "secondo le cronache locali" se serve); i riferimenti alle testate con link stanno **solo in fondo**, tra le fonti di "Per approfondire" (istruzione utente 28/08/2026; versione completa in rule 02 § "Fonti giornalistiche negli articoli"). |
...
🔴 Dal 20/09/2026 Instagram e Facebook pubblicano **da soli** dal repo **privato** `SviluppoItaliaDigitale/social-pc-genzano`, che legge il materiale da `social-bozze/` di questo repo. 🔴 **Dal 21/09/2026 i post escono in concomitanza con l'articolo sul sito**, non a orari fissi: l'innesco è `genera-social-bozze.yml`, che `deploy.yml` chiama **al termine di ogni deploy** e che sveglia il repo privato appena un articolo pronto risponde sul sito; i post distanziati di mezz'ora li chiama `sveglia-social.yml` in modalità attesa (che resta anche come rete due volte al giorno); i post sono distanziati di almeno mezz'ora fra loro e nessuno esce prima che la pagina risponda davvero sul sito. Il sito non cambia: continua solo a generare le bozze. Vincoli scritti nel codice, non nella configurazione:
...
- 🔴 **Gli articoli calendarizzati escono anche loro** (22/09/2026). Il repo privato pubblica solo ciò che ha il materiale in `social-bozze/`, e fino a quel giorno il materiale nasceva solo al commit, quando un articolo scritto in anticipo ha ancora data futura: veniva saltato e nessuno lo riprendeva. Ora nasce quando l'articolo va online (`scripts/social-pronti.py`, testi di riserva dal frontmatter se Gemini non risponde), e il controllo di salute giornaliero segnala ogni articolo online da oltre tre ore rimasto senza materiale o non pubblicato, e i retrodatati (data più vecchia di tre giorni) che il repo privato scarta e vanno pubblicati a mano. 🔴 `workflow_run` non scatta per i run lanciati col `GITHUB_TOKEN`: un innesco "dopo il deploy" si fa con un dispatch esplicito da `deploy.yml`, mai con `workflow_run`. Dettagli in rule 10 § "Pubblicazione automatica social".
...
- **Shortcode:** `foto`, `pittogramma`, `cosa-non-fare`, `chi-chiamare`, `link-card`, `pagina-emergenza-lite`, `dashboard-*` (cruscotto), `scheda-terremoto` (scheda dettaglio evento sismico INGV su `/cruscotto/terremoto/#<id>`), i **dossier interattivi** (`dossier-hero`/`dossier-scena`/`dossier-dati`/`dossier-dato`/`dossier-confronto`/`dossier-hotspot`/`dossier-punto`/`dossier-chiusura`/`dossier-fonti`, sezione `/dossier/` scrollytelling full-bleed), e i **componenti Bootstrap Italia** `callout` (nativo BI), `passi` (stepper), `timeline`, `galleria` (carosello ≥4 foto), e gli shortcode **CAA** `caa-tabella`/`caa-voce` (tabelle di Comunicazione Aumentativa Alternativa: pittogrammi ARASAAC + parola, pagina `/tabelle-comunicazione/`). **Render hook:** `render-link.html`, `render-table.html`. **Partial** chiave: `article-cover`, `leggi-ad-alta-voce` (TTS), `indice-pagina` (indice + scrollspy site-wide), `accessibility-toolbar`, `assistente-fab`, `structured-data` (JSON-LD), `meta-social` (OG), `articoli-correlati`, `page-tools`, `sos-112`, `qr-articolo`, `ricerca-modal` (Pagefind Ctrl+K), `lis-badge`.
- **Pagine/feature speciali:** ricerca Pagefind (`/cerca/`), QR articolo (`static/qr/`), `/stato-sistema/`, `/podcast/` + `/articoli-da-ascoltare/`, `/storia/`, assistente vocale, `/lanterna/` (standalone), LIS `/lis/` (`data/lis.yaml`), "Approfondimenti video" (`genera-video-correlati.py`, gate pertinenza), notifiche allerta browser, quiz `/quiz-preparazione/`, hub `/giochi/`, `/open-data/`, `/audio-e-podcast/`, hub `/standard-iso/`, `/feed-rss/`, traduzioni (hub + 3 pagine × 7 lingue, `tts: false`, hreflang + `<html lang>` dinamico), pagina lite `/emergenza/` (44 KB), metodo editoriale/E-E-A-T (`static/llms.txt`), microtext (`genera-microtext.py`), **cruscotto** `/cruscotto/` (schede dati live INGV/meteo/radar/satellite/aria/mare/ECMWF), **scheda terremoto** `/cruscotto/terremoto/#<id>` (dettaglio di un singolo evento sismico da INGV FDSN, tab sul modello di terremoti.ingv.it, linkata da ogni riga del cruscotto sismico), **Laboratorio meteo** `/laboratorio-meteo/` (costruttore di grafici climatici ERA5/Open-Meteo live + esempi pre-cotti, `static/js/laboratorio-meteo.js`), **Dossier interattivi** `/dossier/` (racconti visivi scrollytelling full-bleed a tema scuro, motore riusabile = sezione + layout + 9 shortcode `dossier-*` + `dossier.css`/`dossier.js`; ogni nuovo dossier = un file in `content/dossier/`; box homepage col più recente; immagini solo con licenza chiara; vedi `manuale/parte-39-dossier-interattivi.md`), **Tabelle di comunicazione CAA** `/tabelle-comunicazione/` (board ARASAAC stampabili per chi in emergenza non riesce a parlare — shortcode `caa-tabella`/`caa-voce`, voce di menu sotto "Accessibilità e Supporti"), **Piano familiare offline** (`/piano-familiare/` — pulsante "Salva piano offline (HTML)": file `.html` autoconsistente solo-testo generato via Blob, apribile senza rete in blackout), **Facile da leggere multilingua** (`/facile-da-leggere/` IT + traduzioni `en`/`eo`/`ro`/`ar` come pagine di sezione, selettore lingua interno + tag `hreflang` SEO reciproci via frontmatter `hreflang:`/`hreflang_xdefault`), e i **banner homepage** `banner-caldo.html` (striscia stagionale "Consigli sul caldo", visibile solo giugno–settembre, mese deciso a build-time) + `banner-volontariato.html` (campagna Regione Lazio "Non c'è PC senza di te", solo modalità ordinaria; vedi memory [[project_campagna_lazio_non_ce_pc_senza_di_te]]).
- **Output format custom** (`hugo.toml [outputs] home`): oltre a HTML/RSS/JSON, la home genera **`/allerta-cap.xml`** (feed **CAP 1.2** Common Alerting Protocol OASIS, machine-readable, da `data/allerta.json` — un `<info>` per pericolo: oggi/domani/avviso meteo/incendi AIB Zona 9; `identifier` composito byte-stabile anti-spam) e **`/news-sitemap.xml`** (sitemap Google News, solo articoli `comunicazioni` delle ultime 48h, dichiarata in `static/robots.txt`).
...
- `aggiorna-manuale.yml` (lunedì 06:04 UTC): monitora le fonti AGID/Designers Italia/DPC e apre issue quando cambiano — manuale e `.claude/rules/` vanno aggiornati in coppia (rule 02 § "Sincronizzazione automatica con gli aggiornamenti AGID").
- `normativa-watcher.yml` (ogni giorno 07:30 italiane) legge **albo pretorio di Genzano, Gazzetta Ufficiale, Normattiva e BURL Lazio** (rassegna stampa il lunedì) e apre la issue del giorno; la **Routine quotidiana «Rassegna normativa e albo pretorio»** (08:00 italiane) la lavora e **pubblica fino a live** un articolo per ogni atto che riguarda la protezione civile direttamente o indirettamente — attivazioni COC comprese (istruzione permanente dell'utente 18/09/2026; rule 10 § omonimo).
