...
9. **BANNER COL TITOLO INTOCCABILE — qualunque sia la fonte della foto**: il banner/copertina dell'articolo deve **SEMPRE** mostrare il **titolo dell'articolo** (cover tipografica con gradiente blu + titolo grande + badge categoria + fascia istituzionale, generata da `genera-cover.py` o `auto-cover-mancanti.py`). La cover tipografica ha **3 funzioni** che giustificano la regola:
   ...
   **TUTTE** le foto — fornite dall'utente, scaricate da Wikipedia/NASA/USGS/NOAA, o stock (Pexels/Pixabay/Unsplash) — vanno **SEMPRE dentro il corpo articolo** con lo shortcode `{{< foto >}}` (mai markdown `![]()` diretto), con fascia blu istituzionale e alt text significativo + caption con credit + licenza. Per **foto multiple in articoli storici** seguire la convenzione: 1ª dopo 1° H2, 2ª dopo 2° H2, foto successive sull'H2 di ogni evento specifico citato. **≥4 foto → galleria/carosello dentro l'articolo** (CSS scoped, cliccabili). **Il campo `image:` deve sempre puntare alla cover tipografica con titolo** (mai vuoto in produzione, mai a foto utente, mai a foto Wikipedia/NASA/altro): il flusso corretto prima del push è lanciare `python3 scripts/auto-cover-mancanti.py` localmente, oppure lasciare `image: ""` E aspettare che `auto-cover-mancanti.py` la generi al run successivo. **Sui social è diverso**: lo script `genera-immagini-social.py` legge le foto inline `{{< foto >}}` dal body e le combina automaticamente in **carosello Instagram** — stessa fonte (foto nel body), due usi distinti, niente da configurare. Mai foto a casaccio.
   ...
   🔴 **ANTI-PATTERN — il campo `image:` non si tocca durante una revisione**. Se il task è *"rivedi questo articolo"*, *"riscrivi secondo AGID"*, *"correggi i refusi"*, *"miglioralo"* o simile, **non hai il permesso di modificare il campo `image:`**, salvo: (a) l'utente te lo chiede esplicitamente; (b) il file puntato non esiste fisicamente → svuotalo a `""`. Errore tipico da non ripetere: trovare una foto pertinente su Wikimedia/NASA/altro durante una revisione e impostarla come `image:` nel banner — la foto va inline come `{{< foto >}}`, `image:` resta `""` o cover tipografica. **Incidente del 9 maggio 2026**: ChatGPT-cloud durante la revisione AGID dell'articolo Giornata Europa ha settato `image: "/images/2026-05-09-ercc-bruxelles.webp"` (foto reale ERCC Wikimedia) anziché lasciare `image: ""` per la cover tipografica. La foto inline era corretta, il banner no. **Check pre-commit obbligatorio:** `git diff <file.md> | grep -E '^[+-]image' | head -5` — se trovi righe `+image:` o `-image:` non esplicitamente richieste dall'utente, **stop**, ripristina prima del commit.
...
16. **KIT CALAMITÀ — 12 kit operativi A4 stampabili per categorie vulnerabili**: a maggio 2026 il Gruppo ha realizzato un'iniziativa autonoma di **schede A4 stampabili** per assistere persone particolarmente vulnerabili durante una calamità. Hub centrale: `/formazione/kit-calamita/` (cruscotto-style "Sono X" → kit specifico). Voce menu: **"Kit pronti per situazioni vulnerabili"** sotto **"Per il Cittadino"** (NON sotto "Per le scuole" — il pubblico target è il cittadino in difficoltà, non l'insegnante). 12 kit attualmente pubblicati: bambini, anziani, RSA, disabilità adulti, neonati, gravidanza, animali, caregiver familiari, pazienti terapie salvavita, senza fissa dimora, italiano L2, volontari PC. Ogni kit cita gli **standard internazionali** di settore (NCTSN PFA, IFRC, WHO, Sphere 2018, FEANTSA, UNHCR CwC, IOM CCCM, IFE Core Group, IAWG MISP, WSAVA, HelpAge, Eurocarers) + normativa italiana (D.Lgs. 1/2018, L. 18/2009, L. 205/2017, L. 281/1991, ARERA, ecc.) + società scientifiche italiane (SIGG, SIN, SIGO, AIOM, AIPO-ITS, CNT, UNIAMO, FISH, FAND, ENPA, ecc.). Materiali **rilasciati gratuitamente**, liberi e riutilizzabili da altri Comuni/Gruppi PC (CC BY-NC-SA 4.0 dove ARASAAC). Citazione del Gruppo molto gradita ma non obbligatoria. **Quando aggiungi un nuovo kit aggiorna 4 punti di accesso**: (a) `_index.md` Hugo del kit, (b) hub `/formazione/kit-calamita/`, (c) `content/mappa-sito/_index.md` (card colorata), (d) assistente virtuale `themes/flavour-pcgenzano/layouts/assistente/list.html` (sotto-albero `kc_*` + opzione in `info_kit_calamita`). Tutte le carte d'identità compilabili usano il template uniforme `.carta-id-*` definito in `static/formazione/kit-calamita-shared/print.css` (riga scrittura 11mm, mai 6mm). I giochi (cruciverba, sudoku) hanno la **soluzione ruotata 180°** in fondo alla stessa scheda (paradigma enigmistico classico, classe `.soluzione-capovolta`) — vietato il vecchio paradigma `<details>` + foglio operatore separato. Specifiche complete in `manuale/parte-20-kit-calamita-categorie-vulnerabili.md`.

19. **TRADUZIONI — `<html lang>` dinamico + hreflang + og:locale (fix 8 maggio 2026)**: le 7 traduzioni del sito (`/english/`, `/francais/`, `/deutsch/`, `/espanol/`, `/portugues/`, `/romana/`, `/esperanto/`) ognuna con 4 sotto-pagine (`_index`, `cosa-fare-adesso`, `numeri-utili`, `piano-familiare`) per un totale di **28 pagine tradotte**. Ogni `_index.md` deve avere nel frontmatter `language: "<codice>"` (en, fr, de, es, pt, ro, eo) — il campo è letto da `baseof.html` per `<html lang>` e da `meta-social.html` per `og:locale` (mappa: en→en_US, fr→fr_FR, de→de_DE, es→es_ES, pt→pt_PT, ro→ro_RO, eo→eo). Senza il campo, fallback a "it" (italiano).

    Il partial `themes/flavour-pcgenzano/layouts/partials/hreflang-tags.html` aggiunge automaticamente `<link rel="alternate" hreflang="...">` per IT + 7 lingue + x-default sulle pagine "tradotte" (`numeri-utili`, `piano-familiare`, `cosa-fare-adesso`, `_index` di ogni lingua). La whitelist delle sezioni tradotte è hardcoded nel partial alla variabile `$sezioniTradotte`. Per aggiungere una nuova pagina tradotta: (a) crea `content/<lingua>/<slug>/_index.md` con `language: "<codice>"`, (b) aggiungi `<slug>` a `$sezioniTradotte` nel partial.

    **Storia del bug**: fino all'8 maggio 2026 tutte le 28 pagine tradotte avevano `<html lang="it">` hardcoded → violazione WCAG 3.1.1 (livello A) + Google le considerava duplicati italiani della pagina principale → 28 pagine in stato N/D ("Rilevate, attualmente non indicizzate") in Google Search Console. Fix risolutivo: campo `language` nel frontmatter + partial hreflang. Dopo il deploy + 1-2 settimane di ricrawl, le pagine devono essere indicizzate correttamente.

17. **FEED RSS PUBBLICI — pagina `/feed-rss/` + 39 feed Hugo auto-generati**: Hugo genera automaticamente un feed RSS 2.0 per ogni sezione del sito (homepage, comunicazioni, allerte, rischi, formazione, glossario, standard ISO, ecc.) — totale **39 feed live su Aruba**, content-type `application/xml`, auto-discoverable via `<link rel="alternate" type="application/rss+xml">` nell'`<head>` (template `baseof.html` riga 45). A maggio 2026 è stata aggiunta una **pagina divulgativa** `content/feed-rss/_index.md` per il cittadino: cosa sono i feed RSS, 3 modi per iscriversi (aggregatori web tipo Feedly/Inoreader, programmi desktop/mobile tipo Thunderbird/NetNewsWire/Read You, auto-discovery browser), elenco dei 14 feed più utili. Voce **"Feed RSS"** nel **footer Hugo** (`hugo.toml [[menus.footer]]` weight 6) + footer `site-chrome.js` (sincronizzato) + card "Strumenti e risorse" della **mappa-sito** + nodo `info_servizi_rss` nell'**assistente virtuale** sotto "Servizi e documenti". Privacy-first: zero registrazione, zero cookie, zero dati personali — solo file XML statici. Niente da configurare per nuove sezioni: Hugo li genera in automatico al build.

18. **STANDARD ISO — hub `/standard-iso/` + serie news divulgative**: a maggio 2026 il Gruppo ha pubblicato una sezione permanente di **30 schede** sugli standard ISO rilevanti per la Protezione Civile, organizzate per famiglia: ISO/TC 292 emergency management (16 standard: 22300-22301-22313-22320-22322-22324-22325-22326-22327-22328-1-22328-2-22329-22351-22361-22395-22398), ISO 31000 risk management family (3), ISO 14090 adattamento climatico (3), ISO 7010 segnaletica (3), ISO 37123/24518/7165/45001/19115 settori connessi (5). Ogni scheda dice cosa è la norma, a chi serve, come si collega alla prassi italiana. Hub: `content/standard-iso/_index.md` con tabella per famiglia. Voce menu: **"Standard ISO"** sotto **"Risorse"** (dropdown) + presente nell'**assistente virtuale** sotto sia `info_servizi` sia `info_accessibilita` + card nella **mappa-sito**.

    ⚠️ **VINCOLO COPYRIGHT FERREO**: gli standard ISO sono **norme tecniche a pagamento**. Le schede contengono **solo titolo, ambito e contestualizzazione italiana**, mai testo della norma. **Mai copiare paragrafi**: solo parafrasi originali basate su titoli/scope pubblici. Per i contenuti dettagliati linkare sempre al sito ufficiale [iso.org](https://www.iso.org/) o al [catalogo UNI](https://store.uni.com/).

    Le **10 news divulgative** correlate sono calendarizzate dal 12 maggio al 12 giugno 2026 (date future, Hugo le esclude dal build attuale, `pubblica-programmata.yml` le rilascia progressivamente al passaggio del giorno). Coprono i 10 standard più utili al cittadino: 22324, 22322, 22329, 22361, 22398, 22395, 31000, 22327, 7010, 14090.

    **Quando aggiungi un nuovo standard ISO aggiorna 4 punti di accesso**: (a) crea `content/standard-iso/iso-XXXX.md`, (b) aggiungilo alla tabella in `content/standard-iso/_index.md` nella famiglia giusta, (c) eventualmente aggiungi card nella `content/mappa-sito/_index.md` se ad alto valore divulgativo, (d) eventualmente aggiungi link nell'assistente `info_servizi_iso` o `info_accessibilita`.
