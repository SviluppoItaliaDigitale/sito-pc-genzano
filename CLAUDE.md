...
9. **FOTO FORNITE DALL'UTENTE — BANNER COL TITOLO INTOCCABILE**: il banner/copertina dell'articolo deve **SEMPRE** mostrare il **titolo dell'articolo** (cover tipografica con gradiente blu + titolo grande + badge categoria + fascia istituzionale, generata da `genera-cover.py` o `auto-cover-mancanti.py`). Le foto fornite dall'utente **NON vanno MAI nel campo `image:`** del frontmatter: vanno **SEMPRE dentro il corpo articolo** con lo shortcode `{{< foto >}}` (mai markdown `![]()` diretto), con fascia blu istituzionale e alt text significativo. Per **foto multiple in articoli storici** seguire la convenzione: 1ª dopo 1° H2, 2ª dopo 2° H2, foto successive sull'H2 di ogni evento specifico citato. **≥4 foto → galleria/carosello dentro l'articolo** (CSS scoped, cliccabili). **Il campo `image:` deve sempre puntare alla cover tipografica con titolo** (mai vuoto in produzione, mai a foto utente): il flusso corretto prima del push è lanciare `python3 scripts/auto-cover-mancanti.py` localmente, oppure lasciare `image: ""` E aspettare che il workflow CI lo faccia. Se serve foto da una delle **7 fonti supportate**, usare il marker `# TODO-foto-{wikipedia,nasa,usgs,noaa,pexels,pixabay,unsplash}: bash scripts/foto-da-XXX.sh "query" slug` (popolerà image: con la foto + credit, NON con cover tipografica). 4 fonti senza API key (Wikipedia, NASA, USGS, NOAA) + 3 stock con API key gratuita configurate via GitHub Secrets `PEXELS_API_KEY`, `PIXABAY_API_KEY`, `UNSPLASH_ACCESS_KEY`. **Sui social è diverso**: lo script `genera-immagini-social.py` legge le foto inline `{{< foto >}}` dal body e le combina automaticamente in **carosello Instagram** — stessa fonte (foto utente nel body), due usi distinti, niente da configurare. Mai foto a casaccio.
