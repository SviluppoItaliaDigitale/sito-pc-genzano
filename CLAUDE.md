...
| `check-allerta.yml` | 4 run/h scaglionati al minuto 7,22,37,52 (`7,22,37,52 * * * *`) | Verifica stato allerta meteo Regione Lazio. Cron espliciti scaglionati fuori dai picchi GitHub Actions (l'inizio orario 0/15/30/45 è il momento più caricato). Verifica empirica 2026-05-10: cron `*/5` non onorato (0 run in 42 min); cron espliciti hanno aderenza molto migliore. Latenza media al cambio livello DPC: ~7-8 min. Commit di `data/allerta.json` resta vincolato a cambio livello effettivo o ultimo_controllo ≥5h45min (max ~4 commit/giorno + cambi reali). |
