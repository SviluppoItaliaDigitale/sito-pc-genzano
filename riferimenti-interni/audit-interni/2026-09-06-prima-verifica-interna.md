...
**Chiuso (decisione dell'utente, 06/09/2026)**: nessun atto disponibile per gli anni Ottanta e Novanta; le nozioni storiche (1981, delibera CC 1991 con il sindaco Cesaroni, delibera CC n. 31/2023) valgono come fonte interna e non vanno più messe in discussione. Registrato in `pc-fact-checker.md` § «Fatti istituzionali del Gruppo confermati dall'utente».
...
Tutti i rilievi della Parte B applicati in un batch dedicato (36 file): acqua 4 L/persona/giorno per 3 giorni e 112 come unico numero di emergenza nelle 7 lingue; `tts: false` sulle 28 pagine; `lang="it"` sui nomi propri italiani; box «Cosa NON fare», 116117, YouPol e chiamata silenziosa riportati dal canonico; blocchi mancanti nelle 4 versioni facili; hreflang reciproci per la famiglia `/facile-da-leggere/` (ramo frontmatter del partial); `og:locale` `ar_AR`; esperanto «Operacia Centro» (il precedente «Salono» significa salotto; «Karabenistoj» confermato da Vikipedio); mappa delle traduzioni riscritta in rule 09 §19 e CLAUDE.md. Chiusi nella stessa serata (PR successiva): indirizzo della sede nel blocco «Vuoi scriverci» delle 4 facili, link a `/aree-attesa/` nella sezione animali, partial hreflang corretto (i path passano ad `absURL` senza barra iniziale, così l'anteprima GitHub Pages conserva il sottopercorso), guardie `try` sulle date in `matrice-territorio.html`, rami morti tolti dallo script Telegram. La pagina lite non carica la toolbar di accessibilità per scelta (ultraleggera), quindi il rilievo «alto contrasto sulla lite» non si applica: le classi della toolbar non arrivano mai a quella pagina.
